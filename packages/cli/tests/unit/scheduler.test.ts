...
describe('startScheduler – interval mode', () => {
  ...
  it('triggers syncFn after the configured interval', () => {
  ...
  it('prevents overlap when syncFn runs longer than the interval', () => {
    const syncFn = vi.fn(() => new Promise<void>(() => {})); // never resolves
    const handle = startScheduler(makeConfig({ interval: 10 }), syncFn);

    vi.advanceTimersByTime(10_000); // first tick → syncFn called, isRunning = true
    vi.advanceTimersByTime(10_000); // second tick → skipped because isRunning
